...
run_metropolized_independent_sampler <- function(data,
  ...
  for (i in 2:n_iters) {
    ...
    q_x <- as.vector(calculate_log_mis_proposal_prob(
      mu_chain[i-1,], rep(0, n_components), proposal_sd^2 * diag(n_components)))
    
    mu_proposal <- rt(n_components, df = proposal_df)
    ...
    q_y <- as.vector(calculate_log_mis_proposal_prob(
      mu_proposal, rep(0, n_components), proposal_sd^2 * diag(n_components)))
